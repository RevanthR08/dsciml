...
def run_forensic_analysis(file_bytes: bytes = None, filename: str = None, return_dict=True):
    ...
    with contextlib.redirect_stdout(capture):
        ...
        COL_MAP = {
            ...
            'event id':      next((c for c in df.columns if c in ['event id', 'eventid', 'event_type', 'event_id']),                   None),
            'user':          next((c for c in df.columns if c in ['user', 'userid', 'accountname']),                        None),
            'opcode':        next((c for c in df.columns if c in ['opcode', 'app_name']),                                   None),
            'task category': next((c for c in df.columns if c in ['task category', 'taskcategory', 'task', 'event_type', 'task_category']), None),
            'computer':      next((c for c in df.columns if c in ['computer', 'device_id', 'machinename']),                                 None),
            'source':        next((c for c in df.columns if c in ['source', 'provider', 'name', 'providername']),            None),
        }
